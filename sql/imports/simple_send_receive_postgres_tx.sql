-- Safe Postgres import for send/receive data (transactional + truncate)
-- This file truncates the target tables (clears existing rows), restarts identities,
-- then inserts the project's sample data, all inside a transaction.
-- IMPORTANT: BACKUP your database before running this. Use the Supabase dashboard or pg_dump.

BEGIN;

-- Remove existing data and reset sequences. CASCADE ensures dependent rows are removed.
TRUNCATE TABLE ratings, submissions, messages, users RESTART IDENTITY CASCADE;
...
-- Advance sequences to match inserted ids
